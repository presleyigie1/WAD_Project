...
fetch('https://ryanair.p.rapidapi.com/flights?origin_code=LGW&destination_code=DUB&origin_departure_date=2023-09-28&destination_departure_date=2023-10-28', options)
	...
	.then(response => {
        ...
        const des2 = document.createTextNode(response.destination_to_origin_trip[1][0].destination_code)
        const timeSlot22 = document.createElement("p")
        const orCode2 = document.createTextNode(response.destination_to_origin_trip[1][0].origin_code)
        const timeSlot32= document.createElement("p")
        const hours2 = document.createTextNode(response.destination_to_origin_trip[1][0].duration_hours)
        const timeSlot42 = document.createElement("p")
        const dep2 = document.createTextNode(response.destination_to_origin_trip[1][0].departure_datetime_utc)
        const timeSlot52 = document.createElement("p")
        const arr2= document.createTextNode(response.destination_to_origin_trip[1][0].arrival_datetime_utc)
        const timeSlot62 = document.createElement("p")
        const currency2 = document.createTextNode(response.destination_to_origin_trip[1][0].currency)
        ...
        displayText3.appendChild(timeSlot3)
        timeSlot3.appendChild(des2)
        //origin code
